...
public class Hex {
    ...
    public static String asUnicode(String s) {
        ...
        for(int i = 0; i < s.length(); i++)
            s1 = s1 + "\\u" + asHex(s.charAt(i) >> 8) + asHex(s.charAt(i));
    ...
    public static String asHex(byte abyte0[]) {
        return asHex(abyte0, abyte0.length).substring(2);
    }
    ...
    */
    public static String dump(byte abyte0[], int i, int j) {
        ...
        if(i < 0)
            i = 0;  //如果起始位置小于0，则设置起始位置为0
        
        int k = Math.min(abyte0.length, i + j);     //取byte数组的长度和起始位置与终止位置之和的最小值
        int l = i & 0xfffffff0;                 //
        int i1 = k + 15 & 0xfffffff0;           //i1表示
